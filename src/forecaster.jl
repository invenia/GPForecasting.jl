...
@auto_hash_equals struct GPForecaster{G<:Grid, T<:Kernel} <: AbstractForecaster
    market::Market{G}
    gp::GP{T, <:Mean}
    training_window::AbstractRange # Range to get training data for
    H_window::AbstractRange # Range to get data for building the mixing matrix
...
function GPForecaster(
    ...
    return GPForecaster(
        ...
        GP(OLMMKernel(
            ...
            Fixed(lat_noise),
            Fixed(H),
            Fixed(Matrix{Float64}(undef, 0, 0)),
            Fixed(Matrix{Float64}(undef, 0, 0)),
            Fixed(Vector{Float64}(undef, 0)),
            [k for i in 1:m],
        )),
        training_window,
        H_window,
