@testset "Parameter" begin
    ...
    @test GPForecasting.name(fx) === nothing
    ...
    @test fv ≈ fv
    @test GPForecasting.unwrap(fv) ≈ v atol = _ATOL_ rtol = _RTOL_
    @test GPForecasting.pack(fv) == Float64[]
    @test GPForecasting.unpack(fv, []) ≈ fv
    @test GPForecasting.name(fv) === nothing
    ...
    @test GPForecasting.name(p) === nothing
    ...
    @test pv ≈ pv
    @test GPForecasting.unwrap(pv) ≈ v atol = _ATOL_ rtol = _RTOL_
    @test GPForecasting.pack(pv) ≈ [-13.815510557964274, 1.0e-6] atol = _ATOL_ rtol = _RTOL_
    @test GPForecasting.unpack(pv, GPForecasting.pack(pv)).p ≈ [2.0e-6, 1.0] atol = _ATOL_ rtol = _RTOL_
    @test GPForecasting.name(p) === nothing
    ...
    for x in range(0., stop=10., length=10)
        b = Bounded(x, 10.)
        @test GPForecasting.unwrap(GPForecasting.unpack(b, GPForecasting.pack(b))) ≈ x  atol = _ATOL_ rtol = _RTOL_
    end
    b = Bounded(-1., 10.)
    @test GPForecasting.unwrap(GPForecasting.unpack(b, GPForecasting.pack(b))) ≈ 0.  atol = _ATOL_ rtol = _RTOL_
    b = Bounded(11., 10.)
    @test GPForecasting.unwrap(GPForecasting.unpack(b, GPForecasting.pack(b))) ≈ 10.  atol = _ATOL_ rtol = _RTOL_
    ...
    @test bv ≈ bv
    @test GPForecasting.unwrap(bv) ≈ v atol = _ATOL_ rtol = _RTOL_
    @test GPForecasting.pack(bv) ≈ [-14.508657238384316, 0.6931470138932831] atol = _ATOL_ rtol = _RTOL_
    @test GPForecasting.unpack(bv, GPForecasting.pack(bv)).p ≈ [4.0, 7.0] atol = _ATOL_ rtol = _RTOL_
